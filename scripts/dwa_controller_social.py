...
class LocalPlannerDWA(Node):
    ...
    def plan_and_execute(self):
        ...
        if self.agents is not None:
            for agent in self.agents.agents:
                ...
                if dist_robot_agent <= THRESHOLD_DIST_STOP_ROBOT :
                    self.get_logger().info("Agent too close, stopping robot.")
                    stop_cmd = Twist()
        ...
        self.get_logger().info("DWA Control...  ")
    ...
    def simulate_trajectories(self, dynamic_window):
        ...
        trajectories = []
        # velocities = np.insert(np.linspace(dynamic_window['v_min'], dynamic_window['v_max'], 7), 0, 0.0)
        velocities = np.linspace(dynamic_window['v_min'], dynamic_window['v_max'], 9)
        for v in velocities:
    ...
    def compute_trajectory_cost(self, trajectory):
        ...
        if len(self.path) == 1:
            ...
            for x, y, _ in trajectory['trajectory']:
                min_dist = mean(math.sqrt((px - x)**2 + (py - y)**2) for px, py in self.path[1:])
                path_cost += min_dist
